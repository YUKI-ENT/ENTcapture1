...
namespace ENTcapture
{
    ...
    public partial class Form1 : Form
    {
        ...
        private Bitmap img, imgorg, imgout;
        ...
        //       public AVIWriter writer = new AVIWriter();
                ...
                try
                {
                    ...
                    while (vcap.IsOpened())
                    {
                        ...
                        if (playmode == 3) //再生中
                        {
                            if (vcap.Read(m))
                            {
                                if (imgorg != null && !lockBmp) imgorg.Dispose();
                                imgorg = BitmapConverter.ToBitmap(m);
                                if (img != null && !lockBmp) img.Dispose();
                                img = (Bitmap)imgorg.Clone();
                                ...
                                if (checkBoxWB.Checked)
                                {
                                    if (filterWB[0] == 1)
                                    {
                                        applyWB(img, filterWB[1], filterWB[2], filterWB[3]);
                                    }
                                    if (filterGamma[1] != 10)
                                    {
                                        applyGamma(img, (float)filterGamma[1] / 10);
                                    }
                                }
                                ...
                                if (SnapFlag)
                                {
                                    Task nowait = SnapBmp(img);
                                    SnapFlag = false;
                                }


                                if (imgout != null) imgout.Dispose();
                                imgout = (Bitmap)img.Clone();

                                //MOUSE 領域指定
                                if (view) //mouse drug 
                                {
                                    DrawRegion(MD, MC, imgout);
                                }
                                else if (checkBoxWB.Checked) // mouse released
                                {
                                    DrawRegion(MD, MU, imgout);
                                }

                                if (!formDisp.Created || formDisp == null)
                                {
                                    SwapImage(pictureBox1, imgout);
                                }
                                else
                                {
                                    formDisp.Invoke(formDisp.dShowBmp, imgout);
                                }
                            ...
                            //計測終了
                            swplay.Stop();
                            long t = swplay.ElapsedMilliseconds;
                            if (t > interval) t = interval;
                            swplay.Reset();

                            //Debug.Print(string.Format("Elapsed time: {0} msec",t));

                            await Task.Delay((int)(interval - t));
                        }
                        else if (playmode == 4) //一時停止中
                        {
                            int tr = this.trackBar1.Value;
                            if (tr != current_frame || checkBoxWB.Checked || SnapFlag) //trackbarを操作した
                            {
                                current_frame = tr;
                                vcap.PosFrames = tr - 1;
                                if (vcap.Read(m))
        ...
        private async void button2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (button2.Text == CaptureButtonOn)
                {
        ...
        private void connectVideo()
        {
            //if (videoSource == null)
            //{
            //    Debug.WriteLine("VideoSource is null. Reconnect to the device.");
        ...
        private void videoRendering(object sender, NewFrameEventArgs eventArgs)
        {
            try
            {
                if (imgorg != null && !lockBmp) imgorg.Dispose();  //WB Filter getRGBaverage内でimgorgのmarshal.copyを行ってるときはDisposeしない
                imgorg = (Bitmap)eventArgs.Frame.Clone();

                if (img != null && !lockBmp) img.Dispose();
                img = (Bitmap)imgorg.Clone();

                //Filter 適用
                if (checkBoxWB.Checked || filterWB[0] == 1)
                {
                    applyWB(img, filterWB[1], filterWB[2], filterWB[3]);
            ...
            {
                MessageBox.Show("描画前フィルタ処理中にエラーが発生しました\r\n" + ex.ToString());
            }
            ...
            {
                message = string.Format("ビデオ描画中にエラーが発生しフレームスキップしました");
                t = 0;
                //MessageBox.Show("ビデオ描画中にエラーが発生しました\r\n" + ex.ToString());
            }
            ...
            try {
                ...
                //videoRecordAccordFFMPEG(img);
            }
            catch (Exception ex)
            {
                MessageBox.Show("ビデオ録画中にエラーが発生しました\r\n" + ex.ToString());
            }
        ...
        private void videoRecord(Bitmap bmp)
        {
            try
            {
        ...
        //private void videoRecordAccordFFMPEG(Bitmap bmp)
        //{
        //    try
        //    {
        //        // delay経過 fpsは取得できてるはず
                ...
                using (Bitmap imgsnap = (Bitmap)bmp.Clone())
                {
                    ...
                    using (var mat_back = new Mat(h, w, MatType.CV_8UC3, new Scalar(255, 255, 255)))
                    {
                            ...
                            if (ol_str[0, 0] != null)
                            {
                                using (Graphics g = Graphics.FromImage(saveimg))
                                {
                                    for (int i = 0; i <= ol_str.Length; i++)
                                    {
                                        ...
                                        s = s.Replace("$i", comboBoxID.Text);
                                        s = s.Replace("$n", comboBoxName.Text);
                        ...
                        catch (Exception)
                        {
                            ...
                            MessageBox.Show("画像切り出し、文字埋込みでエラーが発生しました");
                            lockBmp = false;
                            return;
                        }
                        ...
                        try
                        {
                            if (pictureBox6.Image != null) SwapImage(pictureBox7, (Image)pictureBox6.Image.Clone());
                            if (pictureBox5.Image != null) SwapImage(pictureBox6, (Image)pictureBox5.Image.Clone());
                            if (pictureBox4.Image != null) SwapImage(pictureBox5, (Image)pictureBox4.Image.Clone());
                            if (pictureBox3.Image != null) SwapImage(pictureBox4, (Image)pictureBox3.Image.Clone());
                            if (pictureBox2.Image != null) SwapImage(pictureBox3, (Image)pictureBox2.Image.Clone());
                            ...
                            {
                                pictureBox7.Image = (Image)pictureBox6.Image.Clone();
                                pictureBox6.Image.Dispose();
                            }
                            if (pictureBox5.Image != null)
                            {
                                pictureBox6.Image = (Image)pictureBox5.Image.Clone();
                            ...
                            {
                                pictureBox4.Image = (Image)pictureBox3.Image.Clone();
                                pictureBox3.Image.Dispose();
                            }
                            if (pictureBox2.Image != null)
                            {
                                pictureBox3.Image = (Image)pictureBox2.Image.Clone();
                        ...
                        try
                        {
                            SwapImage(pictureBox2, (Image)saveimg.Clone());
        ...
        private void drawStatus(int mode)
        {
        ...
        private void pictureBox1_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                if (pictureBox1.Image != null) // && mode == 1
                {
                    ...
                    formDisp.Show(this);
                    //formDisp.pictureBox1.SizeMode = PictureBoxSizeMode.Zoom;
                    //formDisp.AutoSize = true;
                    //formDisp.AutoSizeMode = AutoSizeMode.GrowAndShrink;

                    formDisp.Invoke(formDisp.dShowBmp, imgout);
                    //formDisp.pictureBox1.Image = this.pictureBox1.Image;
                }
                else
                {
        ...
        private void applyWB(Bitmap b, int filR, int filG, int filB)
        {
        ...
        private void applyGamma(Bitmap b, float gamma)
        {
